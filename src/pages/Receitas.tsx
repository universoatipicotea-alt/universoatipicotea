...
export default function Receitas({ categorySlug }: { categorySlug?: string }) {
  ...
  return <MemberShell allowGuest eyebrow="Receitas" title="Receitas possíveis para a rotina real." description="Escolha uma receita, descubra novas possibilidades e encontre algo que faça sentido para a sua rotina.">
    ...
    <section><SectionTitle label="Receitas do Universo" title={`${recipes.length} ${recipes.length === 1 ? "receita disponível" : "receitas disponíveis"}`} />
    <Toolbar columns="md:grid-cols-[1.6fr_1fr]">
      ...
      <SelectField label="Filtrar por categoria" value={category} onChange={event => setCategory(event.target.value)}>
