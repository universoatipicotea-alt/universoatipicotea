...
import { BookOpen, CheckCircle2, Eye, FileUp, ImageUp, Lightbulb, Loader2, MessageCircleMore, Save, ShieldAlert, SlidersHorizontal, UsersRound, X } from "lucide-react";
...
export default function Admin() {
  ...
  const [tab, setTab] = useState<"overview" | "guides" | "facilitators" | "moderation">("overview");
  ...
  const uploadFile = async (event: ChangeEvent<HTMLInputElement>, kind: "pdf" | "guideImage" | "facilitatorImage") => {
    ...
    if (file.size > rules.max) { toast.error(`${kind === "pdf" ? "O PDF" : "A imagem"} excede o tamanho permitido.`); return; }
