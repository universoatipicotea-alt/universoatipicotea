...
export default function Library() {
  ...
  const openReader = async (guide: NonNullable<typeof dashboard.data>["guides"][number]) => {
    if (!user) {
      toast.info("Crie uma conta gratuita para ler este PDF, salvar seu progresso e fazer anotações.");
  ...
  return <MemberShell allowGuest eyebrow="Biblioteca" title="Guias para acompanhar sua jornada" description="Materiais práticos, organizados com cuidado, para consultar no tempo e no ritmo que fizer sentido para sua família.">
    <section className="rounded-3xl bg-[var(--sage-deep)] p-6 text-white sm:p-8"><Sparkles size={20} className="text-[#efd4a2]" /><h2 className="display-font mt-5 max-w-2xl text-4xl font-semibold leading-[0.95]">Conteúdo para voltar quando precisar.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-white/75">Todos os guias desta biblioteca são liberados gratuitamente para membros da comunidade.</p></section>
    <section className="mt-10"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><SectionHeading label="Materiais liberados" title={`${availableGuides?.length || 0} guias disponíveis`}
 /><label className="relative block sm:w-72"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--ink-soft)]" size={16} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Buscar na biblioteca" className="h-10 w-full rounded-xl border border-[var(--line)] bg-white pl-9 pr-3 text-sm outline-none focus:border-[var(--sage)]" /></label></div><div className="mt-2 flex gap-2 overflow-x-auto pb-2">{categories.map(category => <button key={category} type="button" onClick={() => setFilter(category)} className={`pressable shrink-0 rounded-full px-3 py-1.5 text-xs font-extrabold ${filter === category ? "bg-[var(--sage-deep)] text-white" : "bg-white text-[var(--ink-soft)] ring-1 ring-[var(--line)] hover:bg-[var(--linen)]"}`}>{category}</button>)}</div>{guides.length ? <div className="mt-6 grid gap-4 md:grid-cols-2">{guides.map(guide => <article key={guide.id} className="soft-card relative overflow-hidden rounded-3xl p-6"><div className="absolute -right-5 -top-5 h-24 w-24 rounded-full bg-[var(--sage-pale)]" /><div className="relative"><div className="flex items-center justify-between gap-4"><span className="rounded-full bg-[var(--sage-pale)] px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.14em] text-[var(--sage-deep)]">{editorialCategory(guide.category)}</span><BookOpen size={20} className="text-[var(--sage-deep)]" /></div><h3 className="display-font mt-6 text-3xl font-semibold leading-[0.98]">{guide.title}</h3><p className="mt-4 min-h-12 text-sm leading-6 text-[var(--ink-soft)]">{guide.summary}</p>{guide.content ? <p className="mt-4 border-l-2 border-[var(--clay)] pl-3 text-xs leading-5 text-[var(--ink-soft)]">{guide.content}</p> : null}<div className="mt-6 flex items-center justify-between gap-3"><span className="inline-flex items-center gap-2 text-xs font-bold text-[var(--ink-soft)]"><Clock3 size={14} />{readingTime(guide)}</span><Button onClick={() => openReader(guide)} disabled={!guide.hasPdf || downloading === guide.id} className="pressable rounded-xl bg-[var(--sage-deep)] text-xs font-extrabold text-white hover:bg-[var(--ink)]"><FileDown size={15} className="mr-2" />{downloading === guide.id ? "Preparando..." : "Ler PDF"}</Button></div></div></article>)}</div> : <div className="mt-6"><ContentEmpty icon={BookOpen} title="Nenhum guia encontrado" text="Tente outro termo ou selecione uma categoria diferente." /></div>}</section>
