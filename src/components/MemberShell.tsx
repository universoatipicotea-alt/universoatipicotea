...
const PREFETCH_BY_HREF: Record<string, Array<[string, unknown]>> = {
  ...
  "/lojamundoazul/membros": [["store.home", { members: true }]],
...
const navigation = [
  ...
  { href: "/lojamundoazul/membros", label: "Loja Mundo Azul", icon: ShoppingBag },
  ...
  const prefetch = () => {
    for (const [path, input] of PREFETCH_BY_HREF[href] ?? []) {
      void queryClient.prefetchQuery({
        queryKey: [path, input],
        queryFn: () => call(path, input ?? undefined),
      });
    }
  };
  return (
    <Link
      ...
      className={`nav-link flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-bold ${collapsed ? "justify-center" : ""} ${active ? "bg-white text-[var(--sage-deep)] shadow-[0_8px_18px_rgba(8,31,77,.08)] ring-1 ring-[var(--line)]" : "text-[var(--ink-soft)] hover:bg-white/75 hover:text-[var(--ink)]"}`}
    >
...
export function MemberShell({
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(
    () =>
      ...
      window.localStorage.getItem("ua-sidebar-collapsed") === "true",
  );
  useEffect(() => {
    window.localStorage.setItem("ua-sidebar-collapsed", String(sidebarCollapsed));
  }, [sidebarCollapsed]);
  if (loading)
    return (
      <div className="min-h-screen bg-[var(--paper)] p-6">
        <div className="mx-auto h-8 max-w-6xl animate-pulse rounded-full bg-[var(--sage-pale)]" />
      </div>
    );
  if (!user && allowGuest)
    return (
      <PublicShell>
        <div className="mx-auto w-full max-w-7xl px-5 py-10 sm:px-8 lg:px-10">
          <div className="mb-10 border-b border-[var(--line)] pb-8">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-[var(--ink-soft)]">{description}</p>
          </div>
          {children}
        </div>
      </PublicShell>
    ...
    return (
      <div className="page-texture grid min-h-screen place-items-center p-6">
        <section className="soft-card w-full max-w-lg rounded-[2rem] p-8 text-center sm:p-12">
          ...
          <Link
            href="/"
            className="mt-5 block text-sm font-bold text-[var(--sage-deep)] underline underline-offset-4"
          >
  ...
  return (
    <div className="min-h-screen bg-[var(--paper)] text-[var(--ink)]">
      <header className="sticky top-0 z-30 border-b border-[var(--line)] bg-[rgba(251,249,245,0.92)] backdrop-blur-xl lg:hidden">
          ...
          <div className="flex items-center gap-2">
            <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
              <SheetTrigger asChild>
                <button
                  type="button"
                  className="grid h-9 w-9 place-items-center rounded-xl border border-[var(--line)] bg-white text-[var(--sage-deep)]"
      ...
      <div className="mx-auto flex w-full max-w-[1760px]">
        <aside
          className={`sticky top-0 hidden h-screen shrink-0 flex-col overflow-hidden border-r border-[var(--line)] bg-[#f7f4ee] p-4 transition-[width] duration-200 lg:flex ${sidebarCollapsed ? "w-[5.5rem]" : "w-80"}`}
        >
          <div
          ...
          >
            ...
            <button
              ...
              className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-[var(--line)] bg-white text-[var(--sage-deep)] shadow-sm transition-colors hover:bg-[var(--linen)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--sage)]"
          ...
          <div className="mt-8 min-h-0 flex-1 overflow-y-auto pr-1">
            <div className="flex flex-col gap-1">
              <p
                className={`mb-3 px-3 text-[10px] font-extrabold uppercase tracking-[0.2em] text-[var(--ink-soft)] ${sidebarCollapsed ? "sr-only" : ""}`}
              >
                Universo
              </p>
          ...
          <div
            className={`mt-4 rounded-2xl border border-[var(--line)] bg-white p-3 ${sidebarCollapsed ? "flex justify-center" : ""}`}
          >
              ...
              {sidebarCollapsed ? null : (
                <>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-extrabold">{user.name || "Membro"}</p>
                    <p className="truncate text-xs text-[var(--ink-soft)]">{accountLabel}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => logout()}
        ...
        <main
          className={`${immersiveMobile ? "bg-[var(--academy-cream)] lg:page-texture" : "page-texture"} min-w-0 flex-1`}
        >
          <div
            className={`mx-auto w-full max-w-[1500px] ${immersiveMobile ? "px-0 pb-0 pt-3 sm:px-8 lg:px-12 lg:py-9" : "px-5 pb-24 pt-7 sm:px-8 lg:px-12 lg:py-9"}`}
          >
            <div
              className={`${compactMobile || immersiveMobile ? "hidden sm:flex" : "flex"} mb-8 items-center justify-between gap-4 rounded-2xl border border-[var(--line)] bg-white/70 px-4 py-3 shadow-sm backdrop-blur sm:px-5`}
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-[var(--ink-soft)]">
                    Receitas, Academia Atípica e comunidade
                  </p>
                </div>
              </div>
              <Link
                href="/perfil"
                className="hidden items-center gap-2 rounded-xl px-2 py-1.5 text-xs font-extrabold text-[var(--ink-soft)] transition hover:bg-white hover:text-[var(--ink)] sm:flex"
              >
                <Initials name={user.name} /> <span>{user.name || "Meu perfil"}</span>
              </Link>
            </div>
            <div
              className={`${compactMobile || immersiveMobile ? "mb-5 px-5 pb-4 sm:mb-10 sm:px-0 sm:pb-8" : "mb-10 pb-8"} ${immersiveMobile ? "hidden sm:block" : "block"} border-b border-[var(--line)]`}
            >
              <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-[var(--sage)]">
                {eyebrow}
              </p>
              <h1
                className={`display-font mt-3 font-semibold ${compactMobile ? "text-3xl sm:text-5xl" : "text-4xl sm:text-5xl"}`}
              >
                {title}
              </h1>
      ...
      <nav
        className={`${immersiveMobile ? "hidden lg:hidden" : "flex"} fixed inset-x-0 bottom-0 z-30 border-t border-[var(--line)] bg-white/95 px-1.5 pt-2 pb-[calc(.5rem+env(safe-area-inset-bottom))] backdrop-blur lg:hidden`}
      >
  ...
  return (
    <div className="mb-5 flex items-end justify-between gap-4">
      <div>
        ...
        <h2 className="display-font text-3xl font-semibold tracking-[-0.03em]">{title}</h2>
      </div>
      {action}
    </div>
  );
...
export function ContentEmpty({
  ...
  return (
    <div className="rounded-3xl border border-dashed border-[var(--line)] bg-white/60 px-6 py-10 text-center">
      <span className="mx-auto grid h-11 w-11 place-items-center rounded-2xl bg-[var(--sage-pale)] text-[var(--sage-deep)]">
        <Icon size={20} />
      </span>
      <h3 className="display-font mt-4 text-2xl font-semibold">{title}</h3>
      <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[var(--ink-soft)]">{text}</p>
    </div>
  );
}
