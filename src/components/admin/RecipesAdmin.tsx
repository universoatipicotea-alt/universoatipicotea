  ...
  return new Promise<void>((resolve, reject) => {
    ...
    request.onload = () =>
      request.status >= 200 && request.status < 300
        ? resolve()
        : reject(new Error("Falha no envio do PDF."));
...
export default function RecipesAdmin({ enabled }: { enabled: boolean }) {
  ...
  const categories = (taxonomy.data?.recipeCategories ?? []).filter(
  ...
  const list = useMemo(() => {
    ...
    const filtered = rows.filter((row) => {
      const matchesTerm =
        !term || `${row.title} ${row.summary}`.toLocaleLowerCase("pt-BR").includes(term);
  ...
  const openEdit = (row: RecipeRow) => {
    setForm({
      ...
      categoryId:
        row.categoryId ?? categories.find((item: any) => item.name === row.category)?.id ?? null,
  ...
  const generateCoverFromPdf = async (source: File | number, baseName: string) => {
    ...
    if (typeof source === "number") {
      const resolved = (await call("community.pdfSource", {
        sourceType: "testGuide",
        documentId: source,
      })) as { url: string };
  ...
  const regenerateCovers = async () => {
    ...
    toast[done ? "success" : "error"](
        ...
        ? `${done} de ${targets.length} capas regeneradas.`
        : "Não foi possível regenerar as capas.",
    );
  };

  const dropHandler =
    (handler: (file?: File | null) => void) => (event: DragEvent<HTMLDivElement>) => {
  ...
  return (
    <section>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--ink-soft)]">
            Crie, organize e publique conteúdos de alimentação.
          </p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            type="button"
            onClick={openNew}
            className="pressable rounded-xl bg-[var(--sage-deep)] text-xs font-extrabold text-white"
          >
      ...
      <div className="mt-6 grid gap-3 rounded-2xl border border-[var(--line)] bg-white p-4 md:grid-cols-[1.4fr_1fr_1fr_1fr]">
        ...
        >
          {["Todas", ...categories.map((item: any) => item.name)].map((item) => (
      ...
      {recipes.isLoading ? (
        ...
        <div className="mt-6 space-y-3">
          {list.map((row) => (
            <article
              key={row.id}
              className="flex flex-wrap items-center gap-4 rounded-2xl border border-[var(--line)] bg-white p-4"
            >
              ...
              <div className="min-w-[200px] flex-1">
                ...
                <h3 className="display-font mt-1 text-xl font-semibold leading-tight">
                  {row.title}
                </h3>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  ...
                  {row.status === "archived" ? "Restaurar" : "Arquivar"}
                </button>
                <button
                  type="button"
                  onClick={() => {
                    if (window.confirm(`Excluir definitivamente "${row.title}"?`))
                      remove.mutate({ kind: "recipe", id: row.id });
                  }}
                  className="inline-flex items-center rounded-xl border border-[var(--line)] px-3 py-2 text-xs font-extrabold text-[#9c583c] hover:bg-[#fdf3ee]"
        ...
        <DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto rounded-3xl border-[var(--line)] bg-[var(--paper)]">
          <DialogHeader>
            <DialogTitle className="display-font text-3xl font-semibold">
              {form.id ? "Editar receita" : "Nova receita"}
            </DialogTitle>
            <DialogDescription>
              Preencha as informações, envie os arquivos e escolha a situação de publicação.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={submit} className="space-y-7">
            <fieldset className="rounded-2xl border border-[var(--line)] bg-white p-5">
              <legend className="px-2 text-[10px] font-extrabold uppercase tracking-[0.16em] text-[var(--sage)]">
                Informações
              </legend>
              <div className="space-y-4">
                <div>
                  <Label htmlFor="recipe-title" className="text-sm font-extrabold">
                    Título
                  </Label>
                  <Input
                    id="recipe-title"
                ...
                <div>
                  <Label htmlFor="recipe-summary" className="text-sm font-extrabold">
                    Resumo
                  </Label>
                  <Textarea
                    id="recipe-summary"
                ...
                <div>
                  <Label htmlFor="recipe-category" className="text-sm font-extrabold">
                ...
                <div>
                  <Label htmlFor="recipe-position" className="text-sm font-extrabold">
                    Ordem
                  </Label>
                  <Input
                    id="recipe-position"
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-sm font-extrabold">
                    Gerada automaticamente a partir da primeira página do PDF.
                  </p>
                  <p className="mt-1 text-xs text-[var(--ink-soft)]">
                    Ao enviar ou substituir o PDF, a capa é recriada sozinha, preservando a
                    proporção original da página. O envio manual de imagem é opcional (JPG, PNG ou
                    WEBP até 6 MB).
                  </p>
            ...
            <fieldset className="rounded-2xl border border-[var(--line)] bg-white p-5">
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-extrabold">
                      {form.pdfKey ? form.pdfName || "PDF enviado" : "Arraste o PDF ou selecione"}
                    </p>
                  ...
                  <div className="flex gap-2">
                    ...
                    {form.pdfUrl ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center rounded-xl border border-[var(--line)] px-3 py-2 text-xs font-extrabold text-[var(--sage-deep)]"
                      >
                ...
                {pdfProgress !== null ? (
                  <div className="mt-3 h-2 overflow-hidden rounded-full bg-[var(--linen)]">
                    <div
                      className="h-full bg-[var(--sage-deep)] transition-all"
              ...
              <div className="mt-4">
                <Label htmlFor="recipe-content" className="text-sm font-extrabold">
            ...
            <fieldset className="rounded-2xl border border-[var(--line)] bg-white p-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <Label htmlFor="recipe-callout" className="text-sm font-extrabold">
                    Destaque da capa{" "}
                    <span className="font-medium text-[var(--ink-soft)]">(opcional)</span>
                  </Label>
                  <Input
                    ...
                    value={form.callout}
                    onChange={(event) =>
                      setForm((current) => ({ ...current, callout: event.target.value }))
                    }
                    maxLength={120}
                    className="mt-2 h-11 rounded-xl border-[var(--line)]"
                  />
                ...
                <div>
                  <Label htmlFor="recipe-accent" className="text-sm font-extrabold">
                    Cor de fundo
                  </Label>
                  <Input
                    id="recipe-accent"
            ...
            <div className="flex flex-wrap justify-end gap-2">
              ...
              <Button
                ...
                className="pressable rounded-xl bg-[var(--sage-deep)] text-xs font-extrabold text-white"
              >
                {save.isPending ? <Loader2 size={14} className="mr-2 animate-spin" /> : null} Salvar
                receita
              </Button>
            </div>
          </form>
